<template>
  <div class="automation-container">
    <div class="title-container">
      <h1 class="section-title">taosync设置</h1>
    ...
    <el-card class="content-card">
      ...
      <el-table v-if="Object.keys(configs).length > 0" :data="configList" style="width: 100%">
        ...
        <el-table-column prop="address" label="taosync地址" />
    ...
    <el-dialog
      ...
      :title="isEdit ? '编辑Taosync配置' : '创建Taosync配置'"
      width="30%"
      :close-on-click-modal="false"
    >
      <el-form :model="form" label-width="120px">
        <el-form-item label="Taosync地址：">
          <el-input v-model="form.address" placeholder="请输入Taosync地址" />
        </el-form-item>
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button type="primary" @click="isEdit ? handleUpdate() : handleCreate()">{{ isEdit ? '保存' : '创建' }}</el-button>
        </span>
      </template>
    </el-dialog>
  </div>
...
const fetchConfigs = async () => {
  try {
    const response = await axios.get('/api/taosync')
...
const handleCreate = async () => {
  ...
  try {
    const response = await axios.post('/api/taosync', form.value)
...
const handleDelete = async () => {
  ...
  try {
    await axios.delete(`/api/taosync/${id}`)
...
const handleUpdate = async () => {
  ...
  try {
    const response = await axios.put(`/api/taosync/${currentId.value}`, form.value)
    configs.value[currentId.value] = response.data
    dialogVisible.value = false
